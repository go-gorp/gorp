...
//
// After the column name, you may add additional configuration preceeded by a
// comma. The available tags are `transient` and `selectonly`. Fields marked 
// `transient` will be treated the same as if you called `SetTransient(true)`
// on the ColMap. Fields tagged `selectonly` will not be included in statements
// generated by gorp, but can still be populated from the database. This is
// useful for expressing relationships that are loaded by a join or subquery.
//
type Product struct {
    Id         int64     `db:"product_id"`
    Price      int64     `db:"unit_price"`
    IgnoreMe   string    `db:"-"`
    Sizes      []*Size   `db:"sizes,selectonly"`
}
```
